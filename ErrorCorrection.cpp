...
int InferPosThreshold( KmerCode &kcode, StoreCML &kmers, int direction, int upperBound ) // -1:left, 1:right
{
	int i ;
	int ret ;
...
bool TestExtend( int t, KmerCode kcode, int depth, int direction, StoreCML &kmers )
{
	int i ;
	KmerCode tmpKcode( kcode ) ;
	if ( depth <= 0 )
...
void SearchPaths_Right( int start, int to, int pos, int t, bool isPaired, char *seq, int fixCnt, int *fix, int fixBottleNeck,
	...
	KmerCode &kcode, StoreCML &kmers, int &trialCnt )
{
	...
	//NOTE: local kmer vs. read threshold determination
	...
	/*for ( i = 0 ; seq[i] ; ++i )
		printf( "%d ", fix[i] ) ;
	printf( "\n" ) ;*/
	//NOTE: check to see if this position is a character to even look at
	if ( nucToNum[ seq[pos] - 'A' ] != -1 )
	{
		...
		if ( cnt >= threshold ) 
		{
			//NOTE: not going to fix this pos
			fix[pos] = -1 ; 
			tmpBottleNeck = fixBottleNeck ;
			if ( cnt < tmpBottleNeck )
				tmpBottleNeck = cnt ;
			++extension ;
			//NOTE: search to the right starting as the next position
		...
		else if ( threshold == 1 && t <= 2 ) 
		{
			...
			if ( k < kmerLength && i < to )
			{
				...
				tmpBottleNeck = fixBottleNeck ;
				//tmpKcode.Prepend( 'A' ) ;
				//NOTE: try to fix further to the right where the cnt is finally above the threshold
...
int ErrorCorrection( char *id, char *seq, char *qual, int pairStrongTrustThreshold, KmerCode &kcode, StoreCML &kmers )
{
	//printf( "Correct %s\n", seq ) ; fflush( stdout ) ;
	...
	//NOTE: drop fold threshold?
	trustThreshold = 2 ;

	//NOTE: set up array of max kmer counts in read
	for ( i = 0 ; i < kcnt ; ++i )
	{
		int t = 7 ;
		if ( kmerLength / 2 > t )
	...
	//NOTE: do the read level threshold
	for ( i = kcnt - 1 ; i >= 1  ; --i )
	{
	...
	//if ( i < kcnt )
	{
		...
		//NOTE: kmer threshold of kmer right b4 dropoff
...
int GetStrongTrustedThreshold( char *seq, char *qual, KmerCode &kcode, StoreCML &kmers ) 
{
	int i, k ;
	int kcnt = 0 ;
	int counts[MAX_READ_LENGTH] ;
...
void GetKmerInformation( char *seq, int kmerLength, StoreCML &kmers, int &l, int &m, int &h )
{
